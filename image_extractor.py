...
def _find_caption(img_top: float, img_bottom: float,
    ...
    Rules (in priority order):
      1. Block starts with a caption prefix  AND  is within CAPTION_GAP_PT below image
      2. Block is the nearest block below image, within CAPTION_GAP_PT,
         horizontally overlapping the image, AND short enough (≤ CAPTION_MAX_CHARS)
    ...
    for blk in text_blocks:
        ...
        # Must be within the vertical gap
        gap = blk_top - img_bottom
        if gap > CAPTION_GAP_PT:
            continue

        # Must have some horizontal overlap with the image
        ...
        if has_prefix or is_short:
            candidates.append((gap, not has_prefix, text))   # sort: gap asc, prefix first

    if not candidates:
        return ""

    # Pick closest candidate; prefer prefix-matched ones at equal distance
