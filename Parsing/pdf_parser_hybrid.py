...
def _build_img_md(img: dict) -> str:
    ...
    The alt text is a short label (Figure N or filename).
    The italic line below is the full caption — only ONE caption line.
    """
    caption   = img.get("caption", "").strip()
    path      = img["path"]
    page      = img["page"]

    # Short alt text: just the figure label e.g. "Figure 1" extracted from caption
    _fig_label = re.match(r'(fig(?:ure)?\s*\d+)', caption, re.IGNORECASE)
    alt = _fig_label.group(1) if _fig_label else f"Figure (page {page})"

    img_block = f"![{alt}]({path})"
    if caption:
        img_block += f"\n*{caption}*"
    return img_block
...
def _inject_images(markdown: str,
    ...
    """
    Inject image references with captions into the markdown.

    Strategy:
      1. Replace each Docling <!-- image --> placeholder (in page order)
         with the corresponding image block.
      2. Any images not matched to a placeholder are appended at the
         correct page position (fallback).
    """
    if not images:
        return markdown, 0
    ...
    # Sort images by page then top (document order)
    sorted_imgs = sorted(images, key=lambda x: (x["page"], x["top"]))

    # ── Step A: Replace <!-- image --> placeholders in order ──────────────
    # Track which images were placed via placeholder
    placed_paths: set[str] = set()

    for img in sorted_imgs:
        if placeholder not in result_md:
            break
        img_block = _build_img_md(img)
        result_md = result_md.replace(placeholder, img_block, 1)
        placed_paths.add(img["path"])
        inserted += 1

    # ── Step B: Fallback — inject remaining images at page position ────────
    for img in sorted_imgs:
        if img["path"] in placed_paths:
            continue   # already injected in Step A
