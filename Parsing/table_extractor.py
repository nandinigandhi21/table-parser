...
def is_continuation(prev_rows, curr_rows, page_diff: int) -> bool:
    ...
    if looks_like_header_repeat(curr_rows[0], prev_rows[0]):
        return False
    # If the current table starts with a header row that matches the previous
    # table's header, it's a NEW table with a repeated header — not a continuation.
    # Also check: if curr first row looks like a standalone header (short words,
    # no numbers) and prev last row is a data row → it's a new table.
    if _looks_like_standalone_header(curr_rows[0]):
        return False
    return True
...
def _looks_like_standalone_header(row: list) -> bool:
    """
    Return True if a row looks like a table header — short non-numeric cells.
    Used to detect the start of a new table rather than a data continuation.
    """
    if not row:
        return False
    ...
    if avg_len > 30:
        return False
    # Header cells are mostly non-numeric
...
def extract_tables(pdf_path: str) -> list[dict]:
    ...
    for i in range(1, len(raw)):
        ...
        if is_continuation(prev["rows"], curr["rows"], curr["page"] - prev["page"]):
            # Strip repeated header from continuation if present
            cont_rows = curr["rows"]
            if cont_rows and looks_like_header_repeat(cont_rows[0], prev["rows"][0]):
                cont_rows = cont_rows[1:]
            if cont_rows:
                logical[-1]["all_rows"].extend(cont_rows)
