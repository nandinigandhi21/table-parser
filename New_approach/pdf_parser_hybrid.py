...
def _inject_images(markdown: str,
    ...
    """
    Inject image references with captions into the markdown at their
    correct page positions.

    Each image is rendered as:
        ![caption or alt](images/pageN_imgM.ext)
        *caption*          (if caption exists)

    Images are inserted after the last content block on their page.
    Docling's <!-- image --> placeholders are replaced first if present,
    otherwise images are appended at their page position.
    """
    if not images:
        return markdown, 0
    ...
    # Step A: Replace Docling's <!-- image --> placeholders in order
    # Docling emits one placeholder per figure — match them to our images
    # by page order.
    placeholder = "<!-- image -->"
    page_image_queues: dict[int, list[dict]] = {}
    for img in sorted(images, key=lambda x: (x["page"], x["top"])):
        page_image_queues.setdefault(img["page"], []).append(img)

    # Replace placeholders page by page in document order
    for img in sorted(images, key=lambda x: (x["page"], x["top"])):
        if placeholder in result_md:
            caption  = img.get("caption", "").strip()
            alt      = caption if caption else f"Figure (page {img['page']})"
            img_md   = f"![{alt}]({img['path']})"
            if caption:
                img_md += f"\n*{caption}*"
            result_md = result_md.replace(placeholder, img_md, 1)
            inserted += 1

    # Step B: Any remaining images whose placeholders weren't found
    # → inject at their page position
    remaining = [
        img for img in sorted(images, key=lambda x: (x["page"], x["top"]))
        if inserted < len(images)
    ]
    # Only inject images that weren't already placed via placeholder
    already_placed = inserted
    for img in sorted(images, key=lambda x: (x["page"], x["top"])):
        img_ref = img["path"]
        if img_ref in result_md:
            continue   # already injected via placeholder replacement
